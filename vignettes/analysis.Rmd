...
lapply(
    topdownr:::.listTopDownFiles(
       ...
       function(x) {
           c(head(paste0(".../", dirname(x), basename(x), 2)), "...")
       }
)
```
...
dataset across all conditions are ETD reagent Target (`EtdReagentTarget`) and
CID activation energy (`CidActivation`),
while ETD reaction time (`EtdReactionTime`)
demonstrates the highest overall impact.
...
![Schema of N-/C-terminal fragments or both.](images/ncb.svg)

In `topdownr` we convert the `TopDownSet` into an `NCBSet` object
(*N*-terminal/*C*-terminal/*B*oth).
...
`2` for a *C*-terminal and a `3` for both fragments.

The `NCBSet` can be used to select the combination of conditions that provide the
best fragment coverage. While computing coverage `topdownr` awards 1 point for
every fragment going from every bond in either *N* or *C* directions.
This means that bonds covered in both directions increase the score of a condition
by 2 points.
For the myoglobin fragmentation example we get the following table for the best three
conditions:
