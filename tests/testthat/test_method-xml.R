...
test_that(".ms2Experiments", {
    ...
    expect_equal(.ms2Experiments(list(A=1:2, B="FOO"),
                 ...
                 list(data.frame(A=c(2, 1, 2, 1), B="FOO",
                                 ...
                                 stringsAsFactors=FALSE)),
                 check.attributes=FALSE)
    expect_equal(.ms2Experiments(list(A=1:2, B="FOO"), groupBy="A",
                                           randomise=FALSE),
...
test_that(".replaceZeroETDReactionTime", {
    ...
    expect_equal(.replaceZeroETDReactionTime(x), r)
})
...
test_that(".xmlHeader", {
    expect_output(.xmlHeader(file=""),
                  "<\\?xml version=\"1\\.0\" encoding=\"utf-8\"\\?>")
    expect_output(.xmlHeader(file="", encoding="utf-16"),
                  "<\\?xml version=\"1\\.0\" encoding=\"utf-16\"\\?>")
})

test_that(".xmlTag", {
    expect_output(.xmlTag("foo", file=""), "<foo/>")
    expect_output(.xmlTag("foo", value="bar", file=""),
                  "<foo>bar</foo>")
    expect_output(.xmlTag("foo", value="bar", indention=2, file=""),
                  "  <foo>bar</foo>")
    expect_output(.xmlTag("foo", value="bar", close=FALSE, file=""),
                  "<foo>bar")
    expect_output(.xmlTag("foo", attrs=c(bar=1), file=""),
                  "<foo bar=\"1\"/>")
    expect_output(.xmlTag("foo", attrs=c(bar=1), close=FALSE,
                                    file=""), "<foo bar=\"1\">")
    expect_output(.xmlTag("foo", value="bar", attrs=c(x=1, y=2),
                                    file=""), "<foo x=\"1\" y=\"2\">bar</foo>")
})

test_that(".xmlTagClose", {
    expect_output(.xmlTagClose("foo", file=""), "</foo>")
    expect_output(.xmlTagClose("foo", indention=2, file=""), "  </foo>")
})

test_that(".xmlListToTags", {
    l <- list(foo="bar", x=1, y=2, z=NA)
    expect_output(.xmlListToTags(l, file=""),
                  "<foo>bar</foo>\n<x>1</x>\n<y>2</y>")
    expect_output(.xmlListToTags(l, indention=2, file=""),
                  "  <foo>bar</foo>\n  <x>1</x>\n  <y>2</y>")
    expect_output(.xmlListToTags(l, na.rm=FALSE, file=""),
                  "<foo>bar</foo>\n<x>1</x>\n<y>2</y>\n<z>NA</z>")
})

test_that(".xmlFullMsScan", {
    expect_output(.xmlFullMsScan(list(FirstMass=100, LastMass=200),
                                           file=""),
...
test_that(".xmlTMSnScan", {
    expect_output(.xmlTMSnScan(data.frame(OrbitrapResolution="R120K",
                                                    ActivationType="CID",
                                                    CollisionEnergy=5,
                                                    stringsAsFactors=FALSE),
                                         mz=c(100, 120),
...
test_that(".xmlCopyAndAppendExperiment", {
    expect_output(.xmlCopyAndAppendExperiment(order=2, src=1, file=""),
...
test_that(".xmlStartEndTime", {
    expect_error(.xmlStartEndTime(order=2, times=1, file=""))
    expect_error(.xmlStartEndTime(order=2, times=1:3, file=""))
    expect_output(.xmlStartEndTime(order=2, idx=1, times=1:2,
                                             file=""),
                  paste0("<Modification Order=\"2\">\n",
...
test_that(".xmlMassList", {
    expect_output(.xmlMassList(c(100, 120), 2:3, file=""),
                  paste0("<MassList>\n",
                         "  <MassListRecord>\n",
                         "    <MOverZ>100</MOverZ>\n",
                         "    <Z>2</Z>\n",
                         ...
                         "  </MassListRecord>\n",
                         "</MassList>"))
    expect_output(.xmlMassList(c(100, 120), 14:15, file=""),
                  paste0("<MassList>\n",
                         "  <MassListRecord>\n",
                         "    <MOverZ>100</MOverZ>\n",
                         "    <Z>10</Z>\n",
                         ...
                         "  </MassListRecord>\n",
                         "</MassList>"))
    expect_output(.xmlMassList(c(100, 120, 140), 2:4,
                                         indention=2, file=""),
                  paste0("  <MassList>\n",
                         "    <MassListRecord>\n",
                         "      <MOverZ>100</MOverZ>\n",
                         ...
                         "    </MassListRecord>\n",
                         "  </MassList>"))
    expect_output(.xmlMassList(c(100, 120), 2:3, 30, type="CID",
                                         file=""),
                  paste0("<MassList CollisionEnergyCID=\"true\">\n",
                         ...
                         "  </MassListRecord>\n",
                         "</MassList>"))
    expect_output(.xmlMassList(c(100, 120), 2:3, 50, type="HCD",
                                         file=""),
...
test_that(".xmlMassListRecord", {
   expect_output(.xmlMassListRecord(100, 3, file=""),
                 paste0("<MassListRecord>\n",
                        "  <MOverZ>100</MOverZ>\n",
                        "  <Z>3</Z>\n",
                        "</MassListRecord>"))
   expect_output(.xmlMassListRecord(100, 13, file=""),
                 paste0("<MassListRecord>\n",
                        "  <MOverZ>100</MOverZ>\n",
                        "  <Z>10</Z>\n",
                        "</MassListRecord>"))
   expect_output(.xmlMassListRecord(100, 3, indention=2, file=""),
                 paste0("  <MassListRecord>\n",
                        "    <MOverZ>100</MOverZ>\n",
                        "    <Z>3</Z>\n",
                        "  </MassListRecord>"))
   expect_output(.xmlMassListRecord(20, 5, energy=30, type="CID",
                                              file=""),
                 paste0("<MassListRecord>\n",
                        "  <MOverZ>20</MOverZ>\n",
                        "  <Z>5</Z>\n",
                        "  <CollisionEnergyCID>30</CollisionEnergyCID>\n",
                        "</MassListRecord>"))
   expect_output(.xmlMassListRecord(20, 5, energy=50, type="HCD",
                                              file=""),
                 paste0("<MassListRecord>\n",
                        "  <MOverZ>20</MOverZ>\n",
                        "  <Z>5</Z>\n",
...
test_that("writeMethodXmls", {
    expect_error(suppressWarnings(writeMethodXmls(list(1:3))),
                 ".*ms1Settings.* has to be a named list")
    expect_error(suppressWarnings(writeMethodXmls(list(foo=1:3))),
                 "is/are no valid MS1 tag")
    expect_error(suppressWarnings(writeMethodXmls(defaultMs1Settings(), list(1:3))),
                 ".*ms2Settings.* has to be a named list")
    expect_error(suppressWarnings(writeMethodXmls(defaultMs1Settings(),
                                           list(foo=1:3))),
    ...
    xml <- c('<?xml version="1.0" encoding="utf-8"?>',
             ...
             '  </Experiment>',
             '</Modification>',
             '<Modification Order="8">',
             '  <Experiment ExperimentIndex="3">',
             '    <StartTimeMin>1.51</StartTimeMin>',
             '    <EndTimeMin>2</EndTimeMin>',
             '  </Experiment>',
             '</Modification>',
             '<Modification Order="9">',
             '  <Experiment ExperimentIndex="4">',
             '    <StartTimeMin>2.01</StartTimeMin>',
             '    <EndTimeMin>2.5</EndTimeMin>',
             '  </Experiment>',
             '</Modification>',
             '<Modification Order="10">',
             '  <Experiment ExperimentIndex="1">',
             '    <TMSnScan>',
             '      <ActivationType>ETD</ActivationType>',
             '      <AgcTarget>10000</AgcTarget>',
             ...
             '      <ActivationType>ETD</ActivationType>',
             '      <AgcTarget>10000</AgcTarget>',
             '      <ETDReactionTime>20</ETDReactionTime>',
             '      <MassList>',
             '        <MassListRecord>',
             '          <MOverZ>100.0003</MOverZ>',
             '          <Z>2</Z>',
             '        </MassListRecord>',
             '      </MassList>',
             '    </TMSnScan>',
    ...
    expect_equal(readLines(file.path(tdir, "method_experiment.xml")), xml)
