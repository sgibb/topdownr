...
test_that(".bestNcbCoverageCombination", {
    ...
    expect_equal(topdownr:::.bestNcbCoverageCombination(m1,
                 ...
                 cbind(index=c(5:4, 1, 3), fragments=c(5:4, 1, 1),
                       bonds=c(4, 2, 1, 0)))
    expect_equal(topdownr:::.bestNcbCoverageCombination(m1, maximise="bonds"),
...
test_that(".highestNcbBondCoverage", {
    m1 <- sparseMatrix(i=c(1:10, 2), j=c(1, 4, 3, 4, 4, 5, 4, 5, 5, 5, 1),
                       x=rep(1, 11))
    expect_error(topdownr:::.highestNcbBondCoverage(as.matrix(1:10)))
    expect_error(topdownr:::.highestNcbBondCoverage(m))
    expect_equal(topdownr:::.highestNcbBondCoverage(m1, intensity=1:5),
                 c(index=5, bonds=4))
    expect_equal(topdownr:::.highestNcbBondCoverage(m1, intensity=5:1),
                 c(index=4, bonds=4))
    expect_equal(topdownr:::.highestNcbBondCoverage(t(m1)),
                 c(index=2, bonds=2))
})
...
test_that(".highestNcbFragmentCoverage", {
    ...
    expect_error(topdownr:::.highestNcbFragmentCoverage(as.matrix(1:10)))
    expect_error(topdownr:::.highestNcbFragmentCoverage(m))
    expect_equal(topdownr:::.highestNcbFragmentCoverage(m1, intensity=1:5),
                 c(index=5, fragments=6))
    expect_equal(topdownr:::.highestNcbFragmentCoverage(m1[,1:3]),
                 c(index=1, fragments=2))
    expect_equal(topdownr:::.highestNcbFragmentCoverage(m1[,1:3], intensity=1:3),
                 c(index=3, fragments=2))
    expect_equal(topdownr:::.highestNcbFragmentCoverage(t(m1)),
