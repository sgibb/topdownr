...
.calculateFragments <- function(sequence, type=c("a", "b", "c", "x", "y", "z"),
    ...
    if (any(c("Met-loss+Acetyl", "Met-loss") %in% modifications)) {
    ...
    } else if ("Met-loss+Acetyl" %in% modifications &&
               startsWith(as.character(sequence), "M")) {
        d <- .unimod766(d, csequence)
    }
    ## TODO: replace by unimod package
...
#' @return modified `data.frame`
#' @noRd
.unimod766 <- function(x, s) {
    ## M already remove by .unimod765
    i <- startsWith(s, x$seq) & grepl("^[AGST]", x$seq)
    x[i, ] <- .unimod1(x[i, , drop=FALSE], s)
    x
}
