...
#'
#' Please note: in `topdownr` version < 1.5 it was used to generate all possible
#' combinations. This interface is still available but deprecated and will be
#' removed in the next bioconductor release (`topdownr` version >= 1.7)
#'
...
writeMethodXmls <- function(ms1Settings, ms2Settings,
                            ...
                            randomise=TRUE, pattern="method-%s.xml",
                            exps,
                            verbose=interactive()) {

    if (missing(exps) && "MethodModifications" %in% names(ms1Settings[[1L]])) {
        exps <- ms1Settings
    }
    if (!missing(exps) && length(exps)) {
